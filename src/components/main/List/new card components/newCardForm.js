...
const isEmpty = (str) => str.replaceAll(' ', '') === '';
const currency = '\u20BD';
...
export default function NewCardForm(props) {
    ...
    const handleButtonClick = () => {
        if(title && price && desc && url) {
            dispatch(addItem(title, url, price, desc, data));
            onClose();
        } else {
            alert(new Error('Please enter all fields').message)
        }

    }
    const handlePrice = ({target: {value}}) => isValid(value) && setPrice(value);
    const handleTitle = ({target: {value}}) => setTitle(value.trim());
    const handleDesc = ({target: {value}}) => setDesc(value.trim());
        ...
        <Dialog open={open} sx={{textAlign: 'center'}}>
            ...
            <div>
                ...
                <TextField
                    ...
                    rows={4}
                    label='Description'
                    placeholder='This impressive paella is a perfect party dish and a fun meal to cook together with your guests. Add 1 cup of frozen peas along with the mussels, if you like.'
                    value={desc}
                    onChange={handleDesc}
                    error={isEmpty(desc)}
                />
